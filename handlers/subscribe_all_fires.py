...
async def handle_sub_to_all_munic(
    message: types.Message, state: FSMContext, session: AsyncSession
):
    user_id = message.from_user.id

    subscribe_query = select(
        Municipalities.municipality_id, Municipalities.municipality_name
    ).order_by(Municipalities.municipality_name.asc())
    ...
    user_subs = select(Subscriptions.municipality_id).where(
    ...
    if user_subs == []:
        for munic in municipality_ids:
            query = (
                ...
                .values(user_id=user_id, municipality_id=munic)
